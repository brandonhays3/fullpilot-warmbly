...
function healthTone(status?: AccountStatus): { dot: string; text: string; label: string; pulse: boolean } {
    ...
    if (h.status === "healthy") return { dot: "bg-emerald-500", text: "text-emerald-600", label: `${h.score}%`, pulse: false };
    if (h.status === "warning") return { dot: "bg-amber-500", text: "text-amber-600", label: `${h.score}%`, pulse: true };
    return { dot: "bg-rose-500", text: "text-rose-600", label: `${h.score}%`, pulse: true };
}
