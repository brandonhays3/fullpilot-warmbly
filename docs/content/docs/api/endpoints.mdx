...
| GET | `/campaigns/:id/analytics/send-methods` | `READ_ANALYTICS` |
...
`GET /campaigns/:id/analytics/send-methods` breaks the campaign's confirmed sends down by the method they took, `{ "campaign_id", "methods": [ { "send_method", "sent", "delivered", "opened", "replied", "bounced", "delivery_rate", "open_rate", "reply_rate", "bounce_rate" } ] }`, most sends first. `send_method` is `<transport>_<provider>_<deployment>` (`smtp_google_cloud_run_worker`), empty for sends confirmed before the method was recorded; rates are percentages of `sent`. `GET /analytics/send-methods?period=7d|30d|90d` is the same breakdown over every campaign send of the workspace, with `period` and `date_range` alongside `methods`. A contact's sent emails (`GET /contacts/:id/emails`) carry each send's `send_method`. See [send methods](/guides/mailboxes/#send-methods).
...
`GET /emails/:id/warmup/instantly` (`READ_EMAILS`) reports the Instantly side of a mailbox's warmup on an instance with `INSTANTLY_API_KEY` set: `configured`, `enrolled` (this mailbox is warmed there), `found` (Instantly's workspace knows the address), and `status` with Instantly's warmup and account state, health score, sent, inbox and spam counts, and the settings applied. Starting warmup answers `409` with `instantly_account_missing` when a Google or Microsoft mailbox has to be added in Instantly first; see [warming through Instantly](/guides/warmup/#warming-through-instantly).
...
| Method | Path | API Permission |
|--------|------|----------------|
| GET | `/analytics/*` (dashboard, deliverability, warmup, campaigns, accounts, usage, send-methods) | `READ_ANALYTICS` |
...
- `GET /auth/instance` (JWT only: the running Warmbly version of a self-hosted instance and whether a newer release exists; a hosted deployment answers `self_hosted: false` and nothing else)
...
The workspace's own OpenRouter key and default model (see the [AI settings](/guides/ai-settings/) guide). Every user-facing generation call (`/generation/*`, reply and compose drafts, AI blocks at send time) runs on this key; without one those calls answer `409` with `code: ai_key_missing`, and a campaign carrying an AI block refuses to start with `campaign_ai_key_missing`. Writing the key takes `manage_settings` for JWT callers or the `AI_AGENT` scope for API keys; reading is open to anyone who may use AI, and never returns the key itself, only whether one is set and its last four characters. `PUT` and `DELETE` are naturally retry-safe.

| Method | Path | Permission |
|--------|------|------------|
| GET | `/organization/current/ai` | `use_ai` / `AI_AGENT` |
| PUT | `/organization/current/ai` | `manage_settings` / `AI_AGENT` |
| DELETE | `/organization/current/ai` | `manage_settings` / `AI_AGENT` |
| GET | `/organization/current/ai/models` | `use_ai` / `AI_AGENT` |

`PUT` takes `api_key` (replaces the key; it is verified against OpenRouter first and refused with `ai_key_invalid`), `model` (the default model id; `""` restores the built-in default), or both. `GET /organization/current/ai/models` proxies OpenRouter's catalog with the workspace key and returns `data`: `id`, `name`, `context_length`, `prompt_per_million` and `completion_per_million` (USD), cached for ten minutes.
