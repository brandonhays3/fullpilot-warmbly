...
Instant sends leave immediately by default (`scheduled_at` is now). If the sender has set an undo window through `PUT /me/send-preferences` (`undo_send_seconds`, 0 to 120, default 0), the send is held that long instead, and within the window it can still be cancelled with `DELETE /unibox/scheduled/:task_id`.
...
{
  "error": "forbidden",
  "message": "Unified Inbox requires an active trial or paid subscription",
  "code": "FORBIDDEN",
