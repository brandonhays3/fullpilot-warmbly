...
| `daily_limit` | integer | no | Accepted for compatibility and ignored. Each mailbox counts its own daily cap. |
...
| `text_only` | boolean | no | Send plain text only: no HTML part, and open and click tracking are off regardless of their flags. Default `true`. |
| `daily_limit` | integer | no | Stored and returned for compatibility, validated `3` to `5000`, and not a cap: the scheduler only consults each mailbox's own daily cap. Defaults to `5000`. |
| `unsubscribe_header` | boolean | no | Stored and returned for compatibility. No `List-Unsubscribe` header is ever sent. Default `false`. |
...
Run the campaign's preflight validation checks (tracking domain, a way to opt out, schedule window, A/B configuration, and more) and return a scored report. No mail is sent. **Scope** `SEND_CAMPAIGNS` · **Org permission** `send_campaigns`.
...
The mailbox may be any active mailbox of the organization, not only one the caller connected, and it does not have to be in the campaign's sender pool or carry its tags. A paused or revoked mailbox is refused with `400`. An API key with an `allowed_email_accounts` list can only test from those mailboxes. Passing `contact_id` additionally requires `READ_CONTACTS` (org permission `view_contacts`), since the rendered copy reads that contact's fields.
...
| `400` | `bad_request` | The campaign has no steps, or the mailbox is not active. |
