...
| `OAUTH_MAILBOX_TRANSPORT` | How Gmail and Outlook mailboxes are driven. `smtp` sends through the provider's SMTP submission endpoint and syncs over its IMAP endpoint, authenticated with the OAuth token (XOAUTH2): `smtp.gmail.com:587` and `imap.gmail.com:993`, `smtp.office365.com:587` and `outlook.office365.com:993`. `api` uses the Gmail API and Microsoft Graph instead. Read by the backend when a mailbox is shipped to a worker and when a consent is started, so it also decides the scopes below | `smtp` |
...
### Scopes and the transport

Every consent asks for `openid email profile` so the mailbox can carry the account's real name. The mailbox scopes depend on `OAUTH_MAILBOX_TRANSPORT`:

| Client | `smtp` | `api` |
|---|---|---|
| `BOX_GOOGLE_*` (web) | the `gmail.*` scopes plus `https://mail.google.com/`, the only Gmail scope IMAP and SMTP accept | the `gmail.*` scopes (`compose`, `metadata`, `modify`, `send`, `settings.basic`, `readonly`) |
| `BOX_GOOGLE_DESKTOP_*` | `https://mail.google.com/` | `https://mail.google.com/` (the Gmail API accepts it too) |
| `BOX_OUTLOOK_*` and `BOX_OUTLOOK_DESKTOP_*` | `offline_access https://outlook.office.com/IMAP.AccessAsUser.All https://outlook.office.com/SMTP.Send` | `offline_access https://graph.microsoft.com/User.Read https://graph.microsoft.com/Mail.Send https://graph.microsoft.com/Mail.ReadWrite` |

Microsoft issues one access token per resource, so a consent never mixes Graph and `outlook.office.com` scopes. That has two consequences when the transport changes. An Outlook mailbox consented under the other transport has to be reconnected from the mailbox drawer before it works again: its token cannot reach the new endpoints, the worker refuses it at load, and the mailbox is marked as needing reconnection. A Gmail mailbox consented through the web client under `api` needs the same, because the granular `gmail.*` scopes do not authenticate IMAP or SMTP; a desktop-client Gmail mailbox carries the full mail scope and moves between transports without reconsent.

On the `smtp` transport the Microsoft tenant must allow SMTP AUTH for the mailbox (it is off by default on some Microsoft 365 tenants), and Gmail and Exchange Online both file a copy of every message submitted over SMTP in the sender's Sent folder, so the worker does not file one of its own.
