...
| `WORKER_DEPLOYMENT` | Where this worker runs, stamped as the last segment of the send method recorded on every send it makes (`smtp_google_cloud_run_worker`). Use `cloud_run_worker` for a rotating Cloud Run Job worker and `cloud_vm` for the persistent worker on the control-plane VM; any `[a-z0-9_]` label works, so a new placement needs no code change | `unknown` | yes |
| `SEND_TRANSPORT_API_PERCENT` | Share (`0` to `100`) of sends from Gmail and Outlook mailboxes that go through the provider API (Gmail API, Microsoft Graph) rather than the provider's SMTP submission endpoint. The path is chosen per send from a hash of the task id, so a retried send takes the same path and the split holds across workers. See [send methods](/guides/mailboxes/#send-methods) | `50` | yes |
| `SEND_FORMAT_HTML_PERCENT` | Share (`0` to `100`) of a plain-text campaign's sends that still carry the HTML part (multipart/alternative); the rest go out as text/plain only. Recorded on each send as `send_format` (`text` or `html`) next to `send_method`, so the four transport-and-format combinations can be compared. A campaign that keeps HTML on always sends what was written | `50` | no |
| `SMTP_ROTATE_AFTER_SECONDS` | Read by the backend. How long an SMTP/IMAP mailbox stays on one ephemeral worker before the rotation pass moves it to another live one. Must be a positive integer | `600` | yes |
| `SEND_ROUND_ROBIN` | Read by the backend. Whether each send (campaign, warmup, test) is routed to the next live worker of the mailbox's tier instead of the worker that syncs it, so consecutive sends from one mailbox leave from different egress IPs. A worker that is not the sync owner is handed the mailbox's credentials for sending only. `false` sends everything through the sync owner. See [send-side routing](/development/architecture/#send-side-routing) | `true` | yes |
...
| `BOX_GOOGLE_REDIRECT_URL`, `BOX_OUTLOOK_REDIRECT_URL` | Present this exact redirect_uri to the provider instead of API base plus the callback path, for a client whose registered redirect lives on a host you control and forward to the callback (query string intact) | unset |
| `BOX_OUTLOOK_DESKTOP_CLIENT_ID`, `BOX_OUTLOOK_DESKTOP_CLIENT_SECRET` | Optional public (desktop-type) Microsoft client; the secret may be empty. Its redirect is `https://localhost`, so the connect modal asks the user to paste the address the consent window lands on. Used as the Outlook path when no `BOX_OUTLOOK_CLIENT_*` is set | unset |
| `BOX_GOOGLE_DESKTOP_CLIENT_ID`, `BOX_GOOGLE_DESKTOP_CLIENT_SECRET` | Optional second Gmail client of Google's desktop type. Google only lets it redirect to localhost, so the connect modal offers it as an alternative and asks the user to paste the address the consent window lands on. Mailboxes remember which client connected them and refresh with the same one | unset |
...
| `OAUTH_MAILBOX_TRANSPORT` | How Gmail and Outlook mailboxes are synced. `smtp` follows the mailbox over the provider's IMAP endpoint, authenticated with the OAuth token (XOAUTH2): `imap.gmail.com:993` and `outlook.office365.com:993`. `api` uses the Gmail API and Microsoft Graph instead. Read by the backend when a mailbox is shipped to a worker. Sends do not follow it: every OAuth mailbox holds both an API client and an SMTP client (`smtp.gmail.com:587`, `smtp.office365.com:587`) and each send picks one by `SEND_TRANSPORT_API_PERCENT` | `smtp` |
...
Every consent asks for `openid email profile` so the mailbox can carry the account's real name, and for the scopes of both send paths, so one consent lets the worker send over the provider API or over SMTP whichever way the split assigns a send:

| Client | Mailbox scopes |
|---|---|
| `BOX_GOOGLE_*` (web) | the `gmail.*` scopes (`compose`, `metadata`, `modify`, `send`, `settings.basic`, `readonly`) plus `https://mail.google.com/`, the only Gmail scope IMAP and SMTP accept |
| `BOX_GOOGLE_DESKTOP_*` | `https://mail.google.com/` (the Gmail API accepts it too) |
| `BOX_OUTLOOK_*` and `BOX_OUTLOOK_DESKTOP_*` | `offline_access https://graph.microsoft.com/User.Read https://graph.microsoft.com/Mail.Send https://graph.microsoft.com/Mail.ReadWrite https://outlook.office.com/IMAP.AccessAsUser.All https://outlook.office.com/SMTP.Send` |

Microsoft issues one access token per resource. The consent covers both resource families at once, but every token request names one: the authorization code is redeemed for a Graph token, which is what is stored, and the worker mints `outlook.office.com` tokens from the refresh token as it needs them, caching each until it expires. A rotated refresh token is persisted with the next Graph token.

A mailbox consented before both families were asked for holds one of the two. The worker finds out at load (it mints one token per resource) and keeps that mailbox on the path it can use, logging the fallback and recording that path as the send method, until the mailbox is [reconnected](/guides/mailboxes/#reconnecting-an-account) from its drawer. The same goes for a Gmail mailbox consented through the web client without `https://mail.google.com/`: it sends through the Gmail API only until reconnected. A mailbox whose token reaches neither path is refused at load and marked as needing reconnection.

For SMTP sends the Microsoft tenant must allow SMTP AUTH for the mailbox (it is off by default on some Microsoft 365 tenants). Gmail and Exchange Online both file a copy of every message submitted over SMTP in the sender's Sent folder, so the worker does not file one of its own.
...
| `INSTANTLY_API_KEY` | An Instantly.ai API v2 key with the `accounts` scopes. When set, starting warmup on a mailbox enrolls it in Instantly warmup through Instantly's API instead of the built-in pool ([guide](/guides/warmup/#warming-through-instantly)). Unset keeps the built-in warmup | unset |
...
| `AI_MODEL_CLASSIFY` | The cheap model campaign replies are classified with (interested, not interested, neutral, auto reply, out of office, unsubscribe). Runs on the consumer for every reply in a campaign conversation; with AI off, or when the call fails, the built-in heuristic classifier decides instead | `openai/gpt-5.6-luna` when `AI_PROVIDER=openrouter`, otherwise the provider's default model |
...
The `AI_*` key is the instance's, not a customer's. It powers reply classification (`AI_MODEL_CLASSIFY`) and the platform-side features (the dashboard assistant, contact research, automation AI nodes, the inbox agent, sequence AI steps). Everything a member generates as their own copy runs on the workspace's own OpenRouter key instead: AI blocks in campaigns, the writing assistant, and reply and compose drafts. Each workspace saves that key under Settings, AI ([AI settings](/guides/ai-settings/)); without one those calls answer `409 ai_key_missing`, and the instance key is never used in its place, so the operator is never billed for a customer's copy.
</Callout>
