...
| `SMTP_EHLO_NAME` | The hostname the worker announces in SMTP EHLO. When unset, a mailbox on a public provider (gmail.com, outlook.com and similar) announces the machine hostname and a custom-domain mailbox announces its own domain. Set it to a hostname that resolves to the worker, for example `mail.example.com` | machine hostname or sender domain |
