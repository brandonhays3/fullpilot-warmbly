...
| `WORKER_DEPLOYMENT` | Where the worker runs, sent on every heartbeat: `cloud_run_worker` (ephemeral, a fresh egress IP per run; SMTP/IMAP mailboxes are placed and rotated on these), `cloud_vm` (persistent, the fallback) or `cloud_dev`. Unset reads as persistent | unset | yes |
