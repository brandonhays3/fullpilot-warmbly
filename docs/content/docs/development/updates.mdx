---
title: Updates
description: How a self-hosted instance learns about a new Warmbly version, where to read that, and how the updater pulls, rebuilds and restarts the stack for you.
---

A self-hosted Warmbly checks for a newer version on its own and reports it through the admin API and the instance health findings. Applying it is one API call when the updater runs next to the stack: the new version is fetched, the services that changed are recreated, and the backend comes back on its own. Nothing in flight is lost: sending and syncing pause for the restart and resume from the database on the other side. Neither the admin panel nor the dashboard carries a version pill or an update dialog.
...
The running build's identity comes from the backend itself, so it says exactly what is deployed, not what the checkout says:

- `warmblyctl status` prints the version, commit and build time as its first line.
- `GET /admin/instance/update` (platform admin, `view_analytics`) returns the running build, the newest release, whether an update is available, and the updater's state.
- An available update is a finding on **Instance > Setup and health** (`update_available`, info severity), so a page that only lists findings still tells you.
- `GET /auth/instance` (any signed-in member) returns the running version and whether a newer release exists, for scripts and integrations.
...
`POST /admin/instance/update/apply` starts the job when the updater is reachable and something newer exists. It needs the `manage_settings` admin permission, and every call is an audit row (`upgrade` on `instance`).
...
`GET /admin/instance/update?log=1` follows the job and returns its log; it answers again once the backend is back from the restart, so a script can poll it until the job reports `succeeded` or `failed`.
...
To leave the updater out entirely, delete the `updater` service from `docker-compose.yml` and set `UPDATER_URL=none` in `.env`.

### Docker Compose from a clone

`make up` starts the stack with the `updater` profile, so a stock install can already apply updates. For a plain `docker compose up`, put this in `.env`:
...
To update by hand instead: `make upgrade` (a fast-forward pull, then `make up`). To leave the updater out entirely, start with `make up UPDATER=false` and set `UPDATER_URL=none` in `.env`; the API then only reports.
...
|---|---|---|
| `UPDATE_CHECK_ENABLED` | Polls GitHub Releases and reports a newer version through the API and health findings | `true` |
...
| `UPDATER_URL` | The updater. Unset, or `none`, leaves the API report-only | `http://updater:8095` under compose |
...
- `updater_unreachable` (warning): `UPDATER_URL` is set but nothing answers there, so the apply endpoint cannot work. Start the updater, fix the address, or remove the variable to update by hand.
