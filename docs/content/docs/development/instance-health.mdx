...
| `GET /admin/instance/update` | `view_analytics` | The running build, the newest release, and the updater's state; see [Updates](/development/updates/) |
...
A newer Warmbly exists: a release on the configured channel is newer than the running build, or the checkout is behind its branch. The message names both versions and how to apply it. With the updater running, `POST /admin/instance/update/apply` does it; without it, `git pull && make up`. See [Updates](/development/updates/).
