...
- the Sunrise Labs org on an active Pro subscription, owned by `sandbox@fullpilot.test`, plus a second teammate (Marco Diaz, admin), one pending invitation, and an active developer API key
...
3. **Unified Inbox**: replies arrive threaded under the original message with full bodies, synced by the worker over IMAP. Out-of-office autoresponders appear but do not count as human replies.
...
**Replies missing from the unified inbox.** Confirm the simulator is running and the consumer is up: delivery is simulator to Dovecot to worker IMAP sync to the event bus to consumer.
