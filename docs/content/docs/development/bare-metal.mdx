...
The updater can run the same script for you. `GET /admin/instance/update` reports when a newer release exists, and with the updater unit installed, `POST /admin/instance/update/apply` pulls, runs the script and reports back. [Updates](/development/updates/#without-docker) has the unit, the sudoers line it needs and the backend variable that points at it.
