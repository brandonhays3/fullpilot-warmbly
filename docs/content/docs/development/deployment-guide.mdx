...
That creates `dev@fullpilot.com` / `password123` along with mailboxes, campaigns, contacts, and two weeks of history. It builds the seed image each time on purpose: `compose run` otherwise reuses a stale one, which fails against a newer database schema.

<Callout type="warn" title="The demo seed plants a published super-admin credential">
It also inserts `admin@fullpilot.local` with a published password, every platform admin permission, and a full-access API key whose secret is a constant in this repository. Never run it on an instance anyone else can reach. Detail in [first run](/development/first-run/#demo-data).
</Callout>
...
The backend tells you when a newer version exists: `GET /admin/instance/update` reports it and it shows as a finding on Setup and health, and `POST /admin/instance/update/apply` pulls the checkout, rebuilds, restarts what changed and comes back once the backend answers. `make up` starts the updater that makes that endpoint work. The whole flow, what it does and how to run it without Docker, is on [Updates](/development/updates/).
