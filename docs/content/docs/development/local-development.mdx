...
Open `http://localhost:5173` and log in with `dev@fullpilot.com` / `password123`.

<Callout type="warn" title="make dev shares its database with make up">
Both pin the same compose project (`-p warmbly`), the same `warmbly_postgres_data` volume and the same `warmbly_dev` database, and `make dev` seeds nine fixture accounts by default, including `admin@fullpilot.local` with every platform admin permission and a published password.
