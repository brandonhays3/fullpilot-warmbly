...
A step has an internal **name** (never seen by recipients), a **subject** with a personalization dialog for merge fields like `{{.FirstName}}` (with a fallback for blank fields, the sender's own name and company, and any custom field), and a rich-text **body**. Selecting a step card opens its pane beside the canvas: the name, the A/B split when the step has variants, and a summary of the email (subject, the first lines of the body, how many files it carries). **Edit email** opens the email in a large two-column editor.

The left column is the full composer: subject, body, templates, Write with AI, and the step's attachments, with a tab per A/B arm along the top. The right column is the email as the recipient will see it, re-rendered as you type through the real send engine, so merge fields, conditionals, and spintax resolve exactly as they will at send time. **Preview as** picks who it renders for: a built-in sample contact, one of the campaign's leads, or any contact you search for, so a custom field your list does not actually have shows up as an unresolved token instead of looking fine. The preview carries the chosen mailbox's signature and From name, the campaign's opt-out footer, and the files the step sends. Malformed templates (an `{{if}}` with no `{{end}}`) are flagged before you start the campaign. See [Personalization](/guides/expressions/) for everything you can put in copy.

Every change in the editor saves to the step on its own; **Done** closes it, and closing with a save still pending asks before discarding. On a phone the two columns become **Edit** and **Preview** tabs.

**Send test email** at the bottom of the editor mails the step to an address of your choice (yours by default) through any active mailbox in the workspace, not only the campaign's own senders, so a mailbox that is not in the pool can still be used to check the copy. The test is rendered for the chosen contact and carries the attachments, signature and opt-out footer; what you wrote is saved first, so the test always matches the editor. Opens and clicks on a test are not tracked, and its opt-out link names nobody, so clicking it suppresses no one. Tests always send the Original arm.
