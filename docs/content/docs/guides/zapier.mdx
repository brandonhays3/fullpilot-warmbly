...
Triggers poll on Zapier's schedule (frequency depends on your plan). Zapier remembers what it has seen, so a Zap runs once per new item. New or Updated Contact re-runs on change, with the real id in `contact_id`. New Email Received needs the Unified Inbox feature, part of an active trial or paid plan.
