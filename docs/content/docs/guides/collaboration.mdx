...
| Surface | Indicator |
|---------|-----------|
| Unified Inbox | Green pulse on a row someone is reading, amber when they are writing a reply, plus a name pill on the open thread, so two people never answer the same prospect |
...
- New mail appears in every member's unified inbox as it syncs, respecting inbox permissions.
...
**Events are permission-aware**: a member without inbox access never receives unified inbox events, and billing events reach only those who can manage billing. See [Team roles](/guides/team-roles/).
