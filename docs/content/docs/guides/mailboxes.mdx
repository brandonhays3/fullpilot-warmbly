...
|----------|--------|-------|
| Gmail / Google Workspace | OAuth (`gmail`) | Recommended. No password stored. Sends through Gmail's SMTP and through the Gmail API, split per send (see [send methods](#send-methods)); syncs over IMAP or the Gmail API depending on how the instance is configured |
| Outlook / Microsoft 365 | OAuth (`outlook`) | Recommended. No password stored. Sends through Exchange Online's SMTP and through Microsoft Graph, split per send; syncs over IMAP or Graph depending on how the instance is configured |
...
**OAuth** sends you to your provider's consent screen and returns a token instead of a password. Consent asks to send mail and to read and organize your mailbox, over the provider's API and over its IMAP and SMTP endpoints alike, plus your name and address so the mailbox is named after you (first and last name as the provider has them; you can rename it afterwards). It needs no app passwords or server settings. Which endpoint syncs the mailbox, the provider's IMAP or its API, is an instance setting ([`OAUTH_MAILBOX_TRANSPORT`](/development/configuration/#mailbox-connections)); how each message is sent is decided per send, see [send methods](#send-methods). A mailbox connected before consent covered both paths keeps working on the path it has and gets the other one after a [re-authorization](#reconnecting-an-account), which also refreshes a mailbox name that was derived from the address. Note that Google revokes Gmail tokens when the account's password changes, so a password change there means re-authorizing the mailbox once.
...
Connecting a mailbox does two things: it imports the mailbox's recent history, and from then on it follows new mail as it arrives. Both are the same on every provider, and both stop at the same line: nothing received before the moment the address was first connected is ever imported or processed.

That moment is remembered per address for the workspace, not per connection. Disconnect a mailbox and connect the same address again, weeks later, and it keeps its original line rather than pulling in whatever arrived while it was gone or the years of history before it. On Gmail the sync asks for mail after that moment, on Outlook the delta and history listings are filtered by received time, and on IMAP the search runs from that date and anything older is skipped by its internal date. A mailbox connected before this line existed has it set to when it was connected.

**The import** brings in the last `90` days, newest first, up to `5,000` messages per mailbox (the defaults; a self-hosted instance can change both under Instance settings), never reaching past the connect line. It reads the inbox, sent mail and archive (plus your own folders on Gmail and IMAP), and skips trash, drafts, spam and Gmail's "All Mail". You can watch it in the mailbox drawer's **Sync** card: a progress bar with a running count while it works, then "Up to date" with what was imported. Because it runs newest first, the mail you would look for is there within the first minute; a large mailbox finishes over the next while at a steady pace.
...
The defaults depend on the provider the mailbox was connected through, because each tolerates a different amount of cold volume from a fresh mailbox:

| Control | Google Workspace / Gmail | Microsoft 365 / Outlook | SMTP/IMAP | Range |
|---------|---------|---------|---------|-------|
| Daily campaign cap | `15`/day | `5`/day | `5`/day | `0` to `5000` |
| Minimum gap between sends | `1200s` (20 minutes) | `600s` (10 minutes) | `600s` (10 minutes) | A hard floor, with jitter added on top |

This is the only daily send cap there is: campaigns have no daily limit of their own, and a campaign sends whatever the mailboxes attached to it allow. The defaults are deliberately conservative. A fresh mailbox should stay there until it has warmed and shown clean complaint and bounce rates, then ramp; `30` to `50`/day is the normal safe band for a mailbox with proven reputation.

The range goes up to `5000` so a high-capacity mailbox (a warmed Google Workspace account allows `2000`/day, Microsoft 365 more) is not artificially blocked, and the dashboard shows a warning on anything above `100`. A high cap only raises the ceiling: the ramp, sending behaviour, and your workspace's daily send limit all still apply, and the smallest one wins. The minimum gap is a throughput bound of its own: at the default `600s` a mailbox tops out around `144` sends in a `24`-hour window, so a cap above that only takes effect together with a shorter gap.
...
The same tab sets the **display name**, **reply-to** (empty uses the mailbox address), **signature** in plain text and HTML, and **tags** for grouping. A newly connected mailbox has no signature: nothing is generated from the display name, and campaign emails carry no signature until you write one here. A new display name is on the From header of the next message the mailbox sends, campaign, reply or warmup alike; nothing needs to be reconnected.
...
A Gmail or Outlook mailbox can hand a message to its provider two ways: through the provider's API (the Gmail API, Microsoft Graph) or through its SMTP submission endpoint with the same OAuth sign-in. The two paths leave the provider's infrastructure differently, and running both side by side has beaten either one alone, so by default half of a mailbox's sends go each way.

The split is per send, not per mailbox. Each send is assigned a path from a hash of its task id, so a send that is retried takes the same path as its first attempt and the share holds across every worker without coordination. Warmup sends follow the same split as campaign sends. The share is an instance setting, [`SEND_TRANSPORT_API_PERCENT`](/development/configuration/#workers), `50` by default; `0` sends everything over SMTP and `100` everything over the API. A plain SMTP/IMAP mailbox has only SMTP. A mailbox whose consent covers only one path (one connected before consent asked for both, or one signed in through Warmbly Cloud) sends over the path it has; the worker logs the fallback, and the mailbox gets both after a [re-authorization](#reconnecting-an-account).

Every confirmed send records the method it took, as `<transport>_<provider>_<deployment>`:

| Segment | Values |
|---------|--------|
| transport | `api` (Gmail API or Microsoft Graph) or `smtp` (the provider's SMTP submission endpoint) |
| provider | `google`, `microsoft` or `smtpimap` |
| deployment | Where the worker that sent it runs, from its [`WORKER_DEPLOYMENT`](/development/configuration/#workers): `cloud_run_worker` for a rotating Cloud Run worker, `cloud_vm` for the persistent worker on the control-plane VM, `unknown` when unset |

So `smtp_google_cloud_run_worker` is a Gmail mailbox sending over SMTP from a Cloud Run worker and `api_microsoft_cloud_vm` an Outlook mailbox sending over Graph from the VM. The method is on each send in a contact's sent-email list (`send_method`), and the campaign overview and the [analytics dashboard](/guides/analytics/#send-methods) break sent, open, reply and bounce rates down by it, which is how you tell whether a path, a provider or a placement is pulling its weight. Sends made before the method was recorded show as not recorded.
