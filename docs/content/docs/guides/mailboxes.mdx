...
|----------|--------|-------|
| Gmail / Google Workspace | OAuth (`gmail`) | Recommended. No password stored. Sends through Gmail's SMTP and syncs over its IMAP with the OAuth token, or over the Gmail API, depending on how the instance is configured |
| Outlook / Microsoft 365 | OAuth (`outlook`) | Recommended. No password stored. Sends through Exchange Online's SMTP and syncs over its IMAP with the OAuth token, or over Microsoft Graph, depending on how the instance is configured |
...
**OAuth** sends you to your provider's consent screen and returns a token instead of a password. Consent asks to send mail and to read and organize your mailbox, plus your name and address so the mailbox is named after you (first and last name as the provider has them; you can rename it afterwards). It needs no app passwords or server settings. Which endpoints the token is used against, the provider's IMAP and SMTP or its API, is an instance setting ([`OAUTH_MAILBOX_TRANSPORT`](/development/configuration/#mailbox-connections)); switching it means [re-authorizing](#reconnecting-an-account) mailboxes consented under the other one, and a reconnect also refreshes a mailbox name that was derived from the address. Note that Google revokes Gmail tokens when the account's password changes, so a password change there means re-authorizing the mailbox once.
