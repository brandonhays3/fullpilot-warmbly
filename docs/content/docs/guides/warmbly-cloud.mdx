...
- your instance gets a mirror of the mailbox with no credential on it. Campaigns, replies and the unified inbox work from your server as before; to send and sync, the instance asks the cloud for a short-lived access token (about an hour, cached) using its link token
