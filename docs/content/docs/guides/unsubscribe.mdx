...
Every recipient of a campaign email can stop hearing from you by replying and saying so, and every opt-out is honoured automatically across the whole workspace. This page covers the mechanisms, the suppression list behind them, and the settings that shape what a recipient sees.

## What a recipient sees

By default, nothing: a campaign email ends with the body and the signature, the way an email from a person does. A reply that asks to stop is detected and honoured automatically (see below), so the opt-out is real without a footer announcing it.

A workspace that wants a visible opt-out can append one after the signature. There are three modes, set for the workspace under **Settings > Sending > Opt-out line** and overridable per campaign under **Preferences**:

| Mode | What is appended | Default |
| --- | --- | --- |
| Nothing | No opt-out in the body | Yes |
| Reply to opt out | A plain sentence you write, for example "If this isn't relevant, just reply and let me know and I won't email you again." Nothing is appended until the sentence is written. | |
...
A campaign with **Plain text only** switched on (the default) ships no HTML at all, so there is nothing for a link to hide inside: link mode and a hand-placed variable both print the full signed address in the copy, which is the one place a cold email least wants a long tracking-shaped URL. The composer warns while you are writing and the launch check warns again before you start.

On a plain-text campaign, leave the opt-out line off or use the reply sentence. Either way a reply that asks to stop is honoured.

## No List-Unsubscribe header

Warmbly never adds the `List-Unsubscribe` and `List-Unsubscribe-Post` headers (RFC 8058) to a campaign email. Those headers are how bulk senders identify themselves to mailbox providers, and a cold email from a person carries none. Google and Yahoo require them only above roughly five thousand messages a day to their consumer inboxes, which no mailbox at Warmbly's limits approaches. The `unsubscribe_header` field on the API is accepted for compatibility and has no effect.
...
Turn this off with **Settings > Sending > Honour replies that ask to stop** if you would rather handle such replies by hand. With no opt-out line in the body, this detection is the recipient's way out, so the launch check warns when both are off. Leaving it off with the reply-to-opt-out line in place means promising an opt-out you then honour manually.
...
| --- | --- |
| Unsubscribed | Replied asking to stop, or clicked the link when a campaign carries one |
...
The list is workspace-wide. An entry stops the address in every campaign, in new campaigns created later, and in the unified inbox composer, which refuses to send to it. Import a list containing a suppressed address and the contact is created but never mailed. The [launch check](/guides/campaigns/#the-launch-check-on-your-list) counts suppressed leads out of a campaign's deliverable total, so what the campaign says it will send is what it sends.
...
An opt-out fires the `campaign.unsubscribed` [webhook](/guides/webhooks/) with a `source` of `link` (the link in the email), `reply` (a reply asking to stop), `action` (a sequence's **Unsubscribe** step) or `one_click` (a mail client's own button on an email sent before the header was retired).
