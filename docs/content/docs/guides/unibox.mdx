...
| Folder | Shows |
| --- | --- |
| Inbox | Campaign conversations: replies to something Warmbly sent, and threads Warmbly started. Everything else a mailbox receives is under **Other** |
...
| View | Shows |
| --- | --- |
| All | Every campaign conversation across every mailbox (except Spam and Trash) |
...
| Other | Everything a mailbox receives that is not part of a campaign conversation: newsletters, notifications, mail from before Warmbly, personal threads |
| Snoozed | Snoozed for later |
| Scheduled | Replies queued to send later |

A conversation is a campaign conversation when a message in it is a reply to something Warmbly sent from that mailbox (matched on the message ids in the reply's headers and on the thread), or when Warmbly started the thread. Only those are classified: the reply class (interested, not interested, out of office, asked to stop, and so on) that drives sequence branching, the inbox agent's drafts, and the reply-based opt-out all run on Inbox mail alone. Mail under **Other** is stored and readable, and you can reply to it, but it is never classified, never suppresses a contact, and never counts as a reply to a campaign. Once you reply to an Other thread from Warmbly, the thread moves to the Inbox.
...
| Control | Behavior |
|---------|----------|
| **Send** | Instant sends leave right away. There is no hold or undo window by default; a per-user window of up to 120 seconds can still be set through the API (`PUT /me/send-preferences`, `undo_send_seconds`), in which case the send sits in the **Scheduled** view until it fires. |
...
Cancel scheduled and smart-queued sends from the **Scheduled** view, or via `DELETE /unibox/scheduled/:task_id`.
