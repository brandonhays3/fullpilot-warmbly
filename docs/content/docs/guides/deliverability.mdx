...
A mailbox on a public mail provider (gmail.com, outlook.com, hotmail.com, yahoo.com, icloud.com, proton.me, gmx, zoho.com, yandex and the like) is never checked and never gated: its records are the provider's, not yours, so there is nothing to fix at a registrar. The drawer shows it as Unknown with the reason "public mail provider, not applicable", and the Advisor raises no authentication finding for it.
...
The gate covers cold campaigns and warmup only. Manual sends and replies from the unified inbox always go out, whatever the domain state, because stopping a human mid-conversation is worse than the reputation risk of one message. The mailbox picker still ranks an unauthenticated mailbox last and labels it.
</Callout>
...
**A way out of every email.** A recipient who replies asking to stop is detected and suppressed automatically, with nothing appended to the email by default. A workspace can add an opt-out line after the signature, a plain "just reply and let me know" sentence or an unsubscribe link, under Settings > Sending. No `List-Unsubscribe` header is ever sent: that header is how bulk senders announce themselves, and a cold email from a person carries none. Someone who can opt out cleanly rarely hits "mark as spam" instead, and every opt-out is suppressed automatically. See [unsubscribe and suppression](/guides/unsubscribe/).
