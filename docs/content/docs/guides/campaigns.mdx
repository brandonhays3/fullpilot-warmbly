...
| --- | --- | --- |
| Per-mailbox cold cap | `15`/day (Google Workspace), `5`/day (Microsoft 365, SMTP/IMAP) | Hard ceiling for cold mail from one mailbox |
| Minimum gap | `1200s` (Google Workspace), `600s` (Microsoft 365, SMTP/IMAP) | Shortest spacing between two sends from one mailbox, always enforced |

The daily cap is a mailbox setting, not a campaign one: a campaign has no daily limit of its own and sends whatever its mailboxes allow. Change a mailbox's cap on its **Settings** tab (see [mailboxes](/guides/mailboxes/#sending-controls)). The `daily_limit` field on the API is accepted for compatibility and ignored.
...
**Preferences** > **Opt-out line** chooses what this campaign appends after the signature: the workspace default from **Settings > Sending**, a reply-to-opt-out sentence, an unsubscribe link, or nothing. Nothing is appended by default; a reply that asks to stop is honoured automatically either way. See [unsubscribe and suppression](/guides/unsubscribe/).
...
The ramp only ever **lowers** a cap, never raises one, so the mailbox's own daily cap still binds. A spam placement pauses the climb for the same three days it pauses the warmup ramp, and the paused days are subtracted rather than made up. The mailbox drawer shows today's allowance and roughly when it reaches your cap.
