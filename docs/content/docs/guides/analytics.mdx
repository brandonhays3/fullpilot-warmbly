...
| Send methods | Sent, open rate, reply rate and bounce rate for each way sends left the workspace in the window, see below |
...
Every confirmed send records how it left: through the provider's API or its SMTP endpoint, from which provider, and from which worker placement (`smtp_google_cloud_run_worker`, `api_microsoft_cloud_vm`; the format is explained in the [mailboxes guide](/guides/mailboxes/#send-methods)). The dashboard and each campaign overview list every method seen with its sends and the share of them delivered (not bounced), opened, replied and bounced, so the two paths can be compared on the same campaigns and the same period. Sends confirmed before the method was recorded are listed as not recorded. The same numbers are on the API as `GET /analytics/send-methods?period=7d|30d|90d` and `GET /campaigns/:id/analytics/send-methods`.
...
**Send methods** breaks the campaign's sends down by the path they took (provider API or SMTP, per provider and worker placement) with sent, open rate, reply rate and bounce rate for each; see [send methods](#send-methods) above.
