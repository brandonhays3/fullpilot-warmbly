...
| Group | Contents |
|-------|----------|
| Workspace | The organization, members, roles, teams, mailboxes (with the moment each address was first connected, so the destination never imports older mail either), API keys, webhooks, and settings, including the website tracking site key. Always included |
...
The workspace's own OpenRouter key ([AI settings](/guides/ai-settings/)) is also left out, together with its default model choice. It is a billing credential for a third-party service, so it stays out of a file that may be handed around; paste it again under Settings, AI on the destination before starting any campaign with AI blocks.
