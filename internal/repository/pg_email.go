...
type EmailRepository interface {
	...
	// SetWarmupProvider records who warms the mailbox (models.WarmupProvider*).
	SetWarmupProvider(ctx context.Context, emailAccountID uuid.UUID, provider string) *errx.Error
...
func (r *emailRepository) ListWarmupScheduleCandidates(ctx context.Context, limit int) ([]uuid.UUID, error) {
	query := `
		...
		WHERE ea.status = 'active'
		  ...
		  AND ea.warmup_provider <> 'instantly'
		  AND (
...
func (r *emailRepository) Search(ctx context.Context, orgID, search string, cursor, tag *string, limit int32, allowedAccountIDs []uuid.UUID) (*models.EmailsResult, *errx.Error) {
	...
	query := `
		SELECT
		 ...
		 ea.warmup_max, ea.warmup_increase, ea.warmup_start_time, ea.warmup_end_time, ea.warmup_days, ea.save_to_sent, ea.warmup_provider,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&i.WarmupStartTime, &i.WarmupEndTime, &i.WarmupDays, &i.SaveToSent, &i.WarmupProvider,
...
func (r *emailRepository) Update(ctx context.Context, userID, emailAccountID string, udata *models.UpdateEmail) (*models.Email, *errx.Error) {
	...
	query := fmt.Sprintf(`
		...
		RETURNING id, organization_id, email, name, signature_plain, signature_html, signature_sync, signature_code, provider, status,
		          ...
		          warmup_start_time, warmup_end_time, warmup_days, save_to_sent, warmup_provider, created_at, updated_at
...
// SetWarmupProvider flips who warms the mailbox. The CHECK constraint on the
// column rejects anything but the known providers.
func (r *emailRepository) SetWarmupProvider(ctx context.Context, emailAccountID uuid.UUID, provider string) *errx.Error {
	query := `UPDATE email_accounts SET warmup_provider = $2, updated_at = now() WHERE id = $1`
	tag, err := r.DB.Exec(ctx, query, emailAccountID, provider)
	if err != nil {
		db.CaptureError(err, query, []any{emailAccountID, provider}, "exec")
		return errx.InternalError()
	}
	if tag.RowsAffected() == 0 {
		return errx.ErrNotFound
...
func (r *emailRepository) GetByID(ctx context.Context, emailAccountID uuid.UUID) (*models.Email, *errx.Error) {
	query := `
		SELECT
		 ...
		 ea.warmup_start_time, ea.warmup_end_time, ea.warmup_days, ea.timezone, ea.save_to_sent, ea.warmup_provider,
	...
	err := r.DB.QueryRow(ctx, query, emailAccountID).Scan(
		...
		&i.WarmupStartTime, &i.WarmupEndTime, &i.WarmupDays, &i.Timezone, &i.SaveToSent, &i.WarmupProvider,
