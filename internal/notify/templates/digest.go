...
{{.Count}} updates in your Fullpilot workspace, bundled into one email.
</p>
{{range .Items}}
...
<a href="{{.AppURL}}" target="_blank" style="display:inline-block;padding:10px 22px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:13px;font-weight:600;color:#ffffff;text-decoration:none;letter-spacing:0.01em;">Open Fullpilot</a>
</td>
</tr>
</table>
...
func GenerateDigestHTML(count int, items []DigestItem) (string, error) {
	...
	return renderEmail(fmt.Sprintf("%d updates in your Fullpilot workspace", count), buf.String())
}
