...
var (
	...
	ErrMailboxNotManaged = errx.NewWithIdentifier(errx.Forbidden, "pool_link_not_managed", "This mailbox's credential is held by the instance, not by Fullpilot Cloud.")
	ErrMailboxBlocked    = errx.NewWithIdentifier(errx.Forbidden, "pool_link_mailbox_blocked", "Fullpilot Cloud has blocked this mailbox for hurting the pool. Sending from it is suspended until it is reviewed.")
	ErrMailboxInactive   = errx.NewWithIdentifier(errx.Forbidden, "pool_link_mailbox_inactive", "This mailbox is not active on Fullpilot Cloud. Reconnect it to keep sending.")
