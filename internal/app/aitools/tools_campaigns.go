...
func (d Deps) registerCampaignTools(r *Registry) {
	...
	r.Register(Tool{
		...
		InputSchema: objectSchema(map[string]any{
			...
			"daily_limit":        intProp("Accepted for compatibility and ignored: each mailbox's own daily cap is the only send cap."),
			...
			"text_only":          boolProp("Send plain text only (the default)."),
			"unsubscribe_header": boolProp("Accepted for compatibility and ignored: no List-Unsubscribe header is ever sent."),
